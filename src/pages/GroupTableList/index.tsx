 ...
 */
  const handleAdd = async (fields: TableListItem) => {
    ...
    try {
      await addGroup({ ...fields }).then((result) => {
        if (result.status == 1) {
          hide();
 ...
 */
  const handleRemove = async (record: TableListItem) => {
    ...
    try {
      removeGroup({ key: record.id }).then((relust) => {
        if (relust.status == 1) {
  ...
  const onFinish = (values: TableListItem) => {
    ...
    try {
      updateGroup(values).then((relust) => {
        if (relust.status == 1) {
          hide();
  ...
  const columns: ProColumns<TableListItem>[] = [
        ...
        rules: [
          {
            required: true,
            message: (
              <FormattedMessage id="pages.searchTable.ruleName" defaultMessage="名称为必填项" />
            ),
          },
        ],
      },
    ...
    },
    {
      ...
      valueEnum: {
        1: {
          text: (
            <FormattedMessage id="pages.searchTable.titleDefaultin.yes" defaultMessage="是" />
          ),
          status: 'yes',
        },
        2: {
          text: (
            <FormattedMessage id="pages.searchTable.titleDefaultin.no" defaultMessage="否" />
          ),
          status: 'no',
        },
      },
    ...
    {
      title: <FormattedMessage id="pages.searchTable.titleType" defaultMessage="类型" />,
      dataIndex: 'type',
        ...
        rules: [
          {
            required: true,
            message: (
              <FormattedMessage id="pages.searchTable.titleType" defaultMessage="类型为必填项" />
            ),
          },
        ],
      },
      ...
      render: (_, record) => (
        <>
          ...
          <a onClick={() => {
            ...
            <FormattedMessage id={"pages.searchTable.GroupSetting"} defaultMessage={"配置"} />
          </a>
        </>
      ),
    },
    ...
    <PageContainer>
      <ProTable<TableListItem>
        headerTitle={intl.formatMessage({
          id: 'pages.searchGroupTable.title',
          defaultMessage: '查询群组信息',
        })}
        actionRef={actionRef}
        rowKey="id"
        search={{
      ...
      <UpdateForm onCancel={() => handleUpdateModalVisible(false)} modalVisible={updateModalVisible}>
        <Form form={appGroupForm} onFinish={onFinish}>
          <Form.Item
            label=""
            name="id"
            style={{ display: "none" }}
      ...
      <SettingForm onCancel={() => handleSettingModalVisible(false)} modalVisible={settingModalVisible} record={record} >
